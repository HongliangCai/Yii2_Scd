...
abstract class ErrorHandler extends Component
{
    ...
    public function register()
    {
        ...
        // 该选项设置是否将错误信息作为输出的一部分显示到屏幕，或者对用户隐藏而不显示。// 关掉php自带的错误显示
        ini_set('display_errors', false);
        //当抛出异常且没有被catch的时候,由yii\base\ErrorHandler::handleException（）处理 // 注册$this->handleException函数为异常处理器，用来处理没有被try catch到异常。
        ...
        //预留内存，防止内存异常的时候错误处理器不能处理错误
        if ($this->memoryReserveSize > 0) {
            // 用string占用一部分内存
            $this->_memoryReserve = str_repeat('x', $this->memoryReserveSize);
        }
        //当出现fatal error的时候由yii\base\ErrorHandler::handleFatalError（）处理  // 注册php运行中止时的处理器
    ...
    {
        // 这个异常是Yii::$app->end()抛出的，不做处理直接返回。
        if ($exception instanceof ExitException) {
            return;
        }
        // 暂存当前正常处理的异常
        $this->exception = $exception;
        // 关掉错误捕获，防止在处理异常时不断获取相同的错误
        // disable error capturing to avoid recursive errors while handling exceptions
        $this->unregister();

        // set preventive HTTP status code to 500 in case error handling somehow fails and headers are sent
        // HTTP exceptions will override this value in renderException()
        // 非命令行应用设置http响应头状态为500
  	    // HTTP exceptions会修改状态码
        if (PHP_SAPI !== 'cli') {
        ...
        try {
            ...
            // 兼容php7,因为php7的异常有error类别的，所以要捕获它们共同的父级 Throwable
     ...
     *
     * @param int $code the level of the error raised. 错误等级
     * @param string $message the error message. 错误信息
     * @param string $file the filename that the error was raised in. 错误文件名
     * @param int $line the line number the error was raised at.  错误发生的行号
     * @return bool whether the normal error handler continues.
     *
     * @throws ErrorException
     */
    public function handleError($code, $message, $file, $line)
    {
        ...
        if (error_reporting() & $code) {
            // 手动加载ErrorHandle，因为在autload的时候发生错误的话就不能自动加载了。
